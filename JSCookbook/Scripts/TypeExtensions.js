 ...
 * @constructor
 */
function NestedError(number, message, innerError) {
...
NestedError.prototype.innerError = undefined;
 ...
 * @constructor
 */
function FormatError(number, message, ctxFormatString, ctxPosition, innerError) {
    var args = new Array();
    for (var i = 0; i < arguments.length; i++)
        args.push(arguments[i]);
	NestedError.apply(this, args.slice(0, 2).concat(args.slice(4)));
	this.formatString = ctxFormatString;
	this.position = ctxPosition;
}
FormatError.prototype = new NestedError();
FormatError.prototype.constructor = FormatError;
FormatError.prototype.formatString = undefined;
FormatError.prototype.position = -1;
 ...
 * @constructor
 */
function KeyNotFoundError(number, message, keyOrIndex, ctxFormatString, ctxPosition, innerError) {
    var args = new Array();
    for (var i = 0; i < arguments.length; i++)
        args.push(arguments[i]);
    FormatError.apply(this, args.slice(0, 2).concat(args.slice(3)));
	this.key = keyOrIndex;
}
KeyNotFoundError.prototype = new FormatError();
KeyNotFoundError.prototype.constructor = KeyNotFoundError;
KeyNotFoundError.prototype.key = undefined;
...
CultureInfo.getValidatedUserLanguage = function(userLanguage, defaultLanguageCode, defaultCountryCode) {
...
Number.prototype.formatString = function (formatSpecifier, precision, zeroPadLength, objCultureInfoOrNumberFormatInfo) {
    ...
    var w = (idx < 0) ? nValue : nValue.substr(0, idx);
    var d = (idx < 0) ? "" : nValue.substr(idx + 1);
    ...
    for (i = w.length; i < vZeroPadLength; i++)
        w = "0" + w;

    if (vPrecision !== null) {
        for (i = d.length; i < vPrecision; i++)
            d += "0";
    }
